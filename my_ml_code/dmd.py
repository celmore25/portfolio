...
class cluster:
	...
	def preprocess(error,locations = np.array([]),
		scale = False, verbose = False, mean_center = False):

		'''
		This will take in error and locations and return a traits matrix that can
		be input into a clustering algorithm. It will also have the types of preprocessing
		we want in there as well.  

		inputs

		param: list error: array of error at each location
		param: list locations: list of lats and longs for each location
		param: bool scale: whether or not to scikit scale the data
		param: bool verbose: verbose argument
		param: bool mean_center: whether or not to mean center the data

		outputs

		return: list traits: processed traits for learning
		'''

		# get the latitude and longitutde
		if locations.any():
			lat = locations[:,0]
			lon = locations[:,1]
		...
		# sclae the data
		if scale:
			error = preprocessing.scale(error)
			if locations.any():
				lat = preprocessing.scale(lat)
				lon = preprocessing.scale(lon)
		if mean_center:
			error = (error - np.mean(error)) / np.mean(error)
			if locations.any():
				lat = (lat - np.mean(lat)) / np.mean(lat)
				lon = (lon - np.mean(lon)) / np.mean(lon)

		# format into traits
		if locations.any():
			traits = np.vstack((error,lat))
			traits = np.vstack((traits,lon)).T
		else:
			traits = np.array(error).reshape(-1, 1)

		return traits


	def analyze(error,locations = np.array([]),N = 4, algorithm = 'KMeans',
		scale = False, verbose = False, mean_center = False, eps = 1):

		'''
		This will use a variety of clustering algorithms to form clusters.
		to try and group indicies into a few different groups that can then
		be analyzed seperately. 

		inputs:
		error - array of errors
		locations - latitude and longitudes
		N - number of groups
		scale - scikit learn scaling
		mean_center - mean_center preprocessing

		outputs:
		groups - array of clustered groups
		'''

		# format the data into a traits matrix
		traits = cluster.preprocess(error,locations)

		# run the cluster analysis
		if algorithm == 'KMeans':
			results = KMeans(n_clusters = N).fit(traits)
		elif algorithm == 'SpectralClustering':
			results = SC(n_clusters = N).fit(traits)
		elif algorithm == 'DBSCAN':
			results = DBSCAN(eps = eps).fit(traits)

		groups = results.labels_

		return groups


	def predict(data,train_start,train_end,num_predict,n_clusters = 3):
		
		'''
		This function will combine a clustering analysis with multiple dmd
		predictions to try and obtain a better prediction.

		inputs:
		data - original price data in a matrix
		train_start - where to start training
		train_end - where to end training
		num_predict - number of time steps in the future to predict
		n_clusters - number of clusters to create

		output: 
		results - class that contains the info below
		error - % error of the new reconstruction
		old_error - % error of the old reconstruction method
		cluster_data - reconstruction with clustering
		cluster_predict - prediction using clustering

		'''

		# first do a sensitivity test
		results = tests.sv_dependence(data,verbose = False)

		# get the error and rank and old error
		old_error = np.min(results.error)
		rank = results.opt_sv_full

		# make an arbitrary time vector
		t = np.arange(int(np.size(data_new[0])))

		# perform the correct decomposition
		dmd_result = dmd.decomp(data, t, num_svd = rank, svd_cut = True)

		# calculate the error (normalized on all points)
		error_mat = 100 * np.absolute(np.real(data - dmd_result.Xdmd) / data)

		# calculate the error at each location
		full_error = []
		...
		groups = self.analyze(full_error, N = n_clusters)

		# separate the data based on the clustering
		data_holder = []
...
class examples:
	...
	def kutz_mr(animate = False, plot_modes = False):
		...
		# add some noise
		Xclean = Xclean + sig*np.random.randn(np.shape(Xclean)[0],np.shape(Xclean)[1])
		...
		mr_dmd = MrDMD(svd_rank=10, max_level=6, max_cycles=2)
		mr_dmd.fit(X=Xclean)
		...
		if animate:
			...
			animation = FuncAnimation(fig, update, interval = 5, frames = np.arange(np.size(time)))
			plt.show()
		...
		if plot_modes:
			...
			im_4 = ax_4.plot(time,np.real(modes[0].A*modes[0].lam**time))
			im_5 = ax_5.plot(time,np.real(modes[1].A*modes[1].lam**time))
			im_6 = ax_6.plot(time,np.real(modes[2].A*modes[2].lam**time))
			plt.show()
...
class dmd:
	...
	def mode_analysis(data,dmd_results, N = np.arange(2), analyze = False, plot = True):
		...
		if plot:
			...
			#loop through the number of modes that are desired to be analyzed
			for ind,n in enumerate(N):
				...
				title = 'Time Mode #'+str(n+1)+' || eig = '+ str(round(dmd_results.omg[n],2))
...
class big_data:
	...
	def make_matrix(folder,file_name,dest):
		...
		np.savetxt(dest+'good_nodes_mod'+'.csv', good_nodes, delimiter='\n', fmt="%s")
...
class cal_plot:
	...
	def plot_cluster(groups,locations):
		...
		data_plot = ax.scatter(lon,lat,alpha = 0.5,c=groups, cmap = cm.tab10, s=8\
			,vmin = 0, vmax = 10)
		ax.axis([-126.30, -104, 32.45, 46.73])
		map_img = mpimg.imread('map.png')
		plt.imshow(map_img, extent=[-126.30, -104, 32.45, 46.73])
